...
class MessageDispatcher:
    '''
    unpack a message and determine the message id, then
    pass the payload to its relevant function, and return the 
    output
    '''
    ...
    def dispatch(self):
        '''
        dispach the payload to the correct function and return result
        '''
        try:
            payload_length, message_id = unpack(">IB", self.payload[:5])
        except Exception as e:
            logging.warning(f'Error when unpacking message {e.__str__()}')
            return None
...
class UdpTrackerAnnounce(Message):
    ...
    def to_bytes(self):
        ...
        msg = (conn_id + action + trans_id, self.info_hash, self.peer_id,
               downloaded + left + uploaded + event + ip + key + num_want,
               port)
...
class Handshake(Message):
    ...
    def __init__(self, info_hash, peer_id=b'-ZZ007-000000000000'):
...
class NotInterested(Message):
    ...
    def from_bytes(cls, payload):
        payload_length, message_id = unpack('>IB', payload[:cls.total_length])
        if message_id != cls.message_id:
            raise WrongMessageException('not a not interested message')
...
class BitField(Message):
    ...
    def to_bytes(self):
        return pack(f'>IB{self.bitfield_length}s', self.payload_length,
                    self.message_id, self.bitfield_as_bytes)
    ...
    def from_bytes(cls, payload):
        ...
        if message_id != cls.message_id:
            raise WrongMessageException('not  bitfield message')
...
class Request(Message):
    ...
    def to_bytes(self):
        return pack('>IBIII', self.payload_length, self.message_id,
                    self.piece_index, self.block_offset, self.block_length)


    @classmethod
    def from_bytes(cls, payload):
        payload_length, message_id, piece_index, \
        block_offset, block_length = unpack('>IBIII', payload[cls.total_length])
        if message_id != cls.message_id:
...
class Piece(Message):
    ...
    def to_bytes(self):
        return pack(f'>IBII{self.block_length}s', 
                    self.payload_length, self.message_id,
    ...
    def from_bytes(cls, payload):
        ...
        payload_length, message_id, piece_index, \
        block_offset, block = unpack(f'>IBII{block_length}s',
                                     payload[:13 + block_length])
        if message_id != cls.message_id:
            raise WrongMessageException('nota a piece message')
...
class Cancel(Message):
    ...
    @classmethod
    def from_bytes(cls, payload):
        payload_length, message_id, piece_index, block_offset,  \
        block_length = unpack(">IBIII", payload[:cls.total_length])
        if message_id != cls.message_id:
